...
export default function SingerPage() {
    ...
    useEffect(() => {
        const fetchApi = async () => {
            get('http://localhost:5050/api/v1/zing/get-singer-by-id' + location.search)
                .then(res => {
    ...
    return (
        <div className='main-home'>
            ...
            <div className='singer-page'>
                <Row gutter={[20, 20]}>
                    <Col span={8}>
                        <img className='singer-header-img' src={singerInfo?.thumbnailM} alt="Avatar" />
                    </Col>
                    <Col span={16}>
                        <div style={{ paddingLeft: 30 }}>
                            <p className='singer-name'>{singerInfo?.name}</p>
                            <p className='singer-birthday'>{singerInfo?.birthday} - {singerInfo?.national}</p>
                            <div style={{ marginBottom: "20px" }}>
                                <span className='singer-follow'>{singerInfo.totalFollow} người quan tâm</span>
                            </div>
                            <div className='singer-biography' dangerouslySetInnerHTML={{ __html: singerInfo.biography }}></div>
                        </div>
                    </Col>
                    <Col span={8}>
                        {singerInfo && (
